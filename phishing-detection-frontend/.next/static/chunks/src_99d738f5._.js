    ...
    const handleCheck = async ()=>{
        ...
        try {
            ...
            const newItem = {
                type: url ? "url" : "email",
                input: url || email,
                result: data.verdict === "safe" ? "Safe ✅" : "Phishing ❌"
            };
            // Update state
            setHistory((prev)=>{
                const updated = [
                ...
                localStorage.setItem("history", JSON.stringify(updated)); // ✅ persist immediately
                return updated;
            });
            // Redirect
            router.push("/result?type=".concat(newItem.type, "&input=").concat(encodeURIComponent(newItem.input), "&verdict=").concat(data.verdict));
            setUrl("");
            setEmail("");
        } catch (error) {
...
_s(Home, "+ok3khI9CAhIlPqKUSC8110Sqjo=", false, function() {
    return [
...
//# sourceMappingURL=src_99d738f5._.js.map